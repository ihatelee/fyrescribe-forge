...
interface EntityVersionHistoryPanelProps {
  entityId: string;
  entityName: string;
  onClose: () => void;
  onRestore: (version: EntityVersion) => Promise<void>;
}
...
const EntityVersionHistoryPanel = ({
  entityId,
  entityName,
  onClose,
  onRestore,
}: EntityVersionHistoryPanelProps) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!cancelled) {
        if (error) console.error("Failed to load entity versions:", error);
        setVersions((data as unknown as EntityVersion[]) ?? []);
        if (isInitial) {
          setLoading(false);
          isInitial = false;
        }
  ...
  const handleRestore = async (v: EntityVersion) => {
    setRestoring(v.id);
    try {
      await onRestore(v);
    } finally {
  ...
  const handleDelete = async (v: EntityVersion) => {
    setDeleting(v.id);
    try {
      ...
      <div className="fixed top-0 right-0 bottom-0 w-full sm:w-[420px] max-w-full bg-fyrescribe-base border-l border-border z-[100] flex flex-col shadow-2xl animate-in slide-in-from-right duration-200">
        ...
        <div className="px-5 py-3 bg-destructive/10 border-b border-destructive/20 flex items-start gap-2">
          ...
          <p className="text-[11px] text-text-secondary leading-relaxed">
            Restoring a version will overwrite the current entity profile.
          ...
          {loading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-xs text-text-dimmed">
                Use the 3-dot menu to save your first version.
              </p>
            </div>
          ) : (
...
export default EntityVersionHistoryPanel;
