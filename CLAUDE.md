...
- **Outrun visual overhaul** — When theme is "outrun": logo swaps to `fyrescribe_logo_bit.svg` (fill set to `#00FF41` directly in the SVG) in `Titlebar` and `OnboardingPage`; sparkle animation is `OutrunGridBackground` (CSS `perspective`/`rotateX` grid with animated `background-position-y` scroll, horizon glow line removed, all colors from `--gold` CSS variable); outrun palette includes `--neon-yellow: 72 100% 50%` (#CCFF00, acid yellow-green), used for: active nav item border + text in `Sidebar`, play button + volume slider in `OutrunMusicPlayer`.
- **Outrun music player** — `OutrunMusicPlayer` component rendered in `Sidebar.tsx` only when theme is "outrun". Uses `position: absolute; bottom: 0; left: 0; right: 0` within the sidebar's fixed panel (`position: fixed` acts as the containing block). The sidebar's bottom nav section gets `pb-[72px]` when outrun is active to prevent overlap. `AppLayout` has no extra panel or padding for the player. Streams from `OUTRUN_MUSIC_URL` constant (currently Nihilore – Motion Blur). Auto-plays on mount (theme activation); pauses on unmount (theme change). Gracefully falls back to paused state if browser autoplay is blocked. Controls: play/pause (neon yellow) + volume slider (neon yellow fill). Credit: "♪ Nihilore". Track URL is a clearly commented constant at the top of `OutrunMusicPlayer.tsx`.
